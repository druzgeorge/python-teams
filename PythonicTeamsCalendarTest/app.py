...
def index():
    ...
    month_events = helpers.retrieve_all_events_in_month(Month=current_month)
    print(month_calendar)
    return render_template('calendar(1).html', current_month=current_month, current_year=current_year, month_calendar=month_calendar, month_events=month_events)
...
def view_event(day, month, year):
    events_list = helpers.retrieve_events_on_date(Day=day, Month=month, Year=year)
